...
public class Ch06AdvancedClass {
    ...
    static void changeParcelStatus(ParcelStatus afterParcelStatus, String dateMessage) {
        ...
        if (parcel.getParcelStatus() != ParcelStatus.접수) {
            System.out.println("접수 상태의 택배만 " + afterParcelStatus + " 처리할 수 있습니다.");
            return;
        }
...
class Parcel {
    ...
    public Parcel(
        ...
        fee = calculateFee();
        // 9. 배송예정일자
        expectedDeliveryDate = calculateExpectedDeliveryDays();
    }
    ...
    int calculateFee() {
        ...
        if (getDeliveryType() == DeliveryType.특급) {
            deliveryFee += 2000;
        } else if (getDeliveryType() == DeliveryType.냉장) {
            deliveryFee += 4000;
        } else if (getDeliveryType() == DeliveryType.해외) {
            deliveryFee += 15000;
        }
...
//        getHistories()[getHistoryCount()] = deliveryHistory;
